...
> **For agentic workers:** 실행 시 `superpowers:executing-plans`를 사용해 아래 작업을 순서대로 수행한다. 작업 상태는 체크박스로 기록한다. 1차(Task 1–4) 구현 완료. 실행 판단과 검증은 docs/stabilization-progress.md에 기록한다.
...
- [x] 기존 20개 테스트와 빌드를 다시 확인하고 현재 구현을 별도 기준선 커밋으로 보존한다. 사용자 변경이 있으면 포함 범위를 확인한다.
- [x] 인계 불변성·명시적 공유·모델 우선순위·폐기 대상 제한 회귀 테스트를 유지한다.
- [x] actor를 저장 데이터의 session에서 분리하여 CommandContext로 받도록 정리한다. 역할별 조회 selector를 공통으로 사용한다.
- [x] UI의 boolean dispatch 의존 지점을 목록화하고 비동기 CommandResult를 처리하는 경계로 전환한다. 실패 시 다이얼로그/초안을 보존한다.
- [x] 관련 테스트와 빌드를 통과한 변경을 독립 커밋한다.
...
- [x] fake-indexeddb로 두 독립 클라이언트의 메모 추가·동일 레코드 충돌 테스트를 먼저 작성한다.
- [x] Dexie 테이블에 에이전트/프로필/업무/대화/메시지/파일버전/Blob/묶음/인계/SR/이벤트/알림/요청/완료스냅샷/명령영수증을 저장한다.
- [x] append는 모두 보존하고, 편집은 revision 비교로 충돌을 알린다. 오래된 전체 상태를 DB에 덮어쓰지 않는다.
- [x] Blob과 메타데이터를 같은 DB 트랜잭션에 저장한다. 구독에는 liveQuery 집계 조회를 사용한다(엔티티별 화면 구독 최적화는 실행 기록의 판단 참조).
- [x] 새 DB `mes-agent-hub-v2`로 기존 localStorage/Blob DB 데이터를 복사한다. ID와 출처를 보존하고 검증 후 완료 마커를 기록한다.
- [x] 이관 중 탭이 여러 개 열려도 하나의 완료 마커만 확정한다. 실패/용량 부족/누락 Blob은 기존 원본을 남기고 복구 안내한다. 자동으로 빈 시드로 덮어쓰지 않는다.
- [x] 기존 저장소는 읽기 전용 원본으로 남긴다. v2가 만들어진 후 중복 가져오기를 하지 않는다. 구버전 앱을 동시에 쓰지 않도록 버전 안내한다.
- [x] 탭 사용자/현재 화면 설정은 sessionStorage로 분리한다. 키는 메모리 유지.
- [x] 이관 반복, 누락 자료, 중간 실패, 충돌 회귀 테스트 및 두 탭 UI 시연 후 커밋한다.
...
- [x] 응답 대기 중 컨텍스트 제거, 화면 이동, 역할 변경, 동시 시작, 만료 후 늦은 응답 테스트를 실패 상태부터 작성한다.
- [x] RequestRecord에 작성자·공개 범위·모델 설정·선택 버전·시각·상태·lease token을 저장한다.
- [x] 전송 시작 트랜잭션에서 사용자 메시지와 요청을 먼저 저장하고 lease를 획득한다. 네트워크 호출은 DB 트랜잭션 밖에서 수행한다.
- [x] 완료는 requestId를 기준으로 응답/상태/활동을 원자적으로 저장한다. 활성 컨텍스트가 바뀌어도 과거 기록을 수용한다.
- [x] 실패/중단/취소 기록과 사용자 재시도를 제공한다. 재시도는 새 requestId와 이전 요청 참조를 만든다. 자동 중복 전송은 하지 않는다.
- [x] 요청 본문 크기 한도, 불변 버전 참조와 텍스트 조각 공유 저장을 적용한다. 같은 Blob 중복 저장/요청 재구성 일치를 검증한다.
- [x] 앱 재시작으로 중단된 요청은 interrupted로 남긴다. 서버 작업의 취소나 exactly-once 실행을 보장한다고 표시하지 않는다.
- [x] 요청 처리 회귀 테스트·모의 느린 API 시연·빌드 후 커밋한다.
...
- [x] 완료 자료 수정 차단, 후속 메모 허용, 재개 사유, 실행 중 요청 존재 시 완료 차단 테스트를 작성한다.
- [x] CompletionSnapshot과 구조화된 완료/재개 이벤트를 구현한다. 여러 번 완료해도 스냅샷을 덮어쓰지 않는다.
- [x] 관리자 데이터 관리에서 전체 백업과 가져오기 미리보기를 제공한다. 스키마·ID중복·참조·Blob해시·인코딩·용량·버전을 검증한다.
- [x] 유효하지 않은 백업은 현재 데이터를 건드리지 않는다. 정상 복원은 원자적 전체 교체로 처리하며 실행 중 요청이 있으면 먼저 중단하도록 안내한다.
- [x] session/API키/활성 lease는 백업 대상에서 제외한다. 복원된 pending 요청은 interrupted로 전환한다.
- [x] 원본 파일을 포함한 왕복 복원·오류 롤백·키 미포함 테스트와 빌드 후 커밋한다.
